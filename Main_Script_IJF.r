...
forecast_origins<-rownames(KNN_results_forecasts$KNN_errors)
par(mfrow=c(3,3),mar=c(4,4,1,1))
k_values<- 52
lead_times<- 1:9
lwd_width<-3
for(i in lead_times){
    plot(1:k_values,KNN_results_forecasts$k_best[[i]][1,],type="l",xlab="K",xlim=c(1,k_values),
         ylab="MAPE (%)",col="blue",ylim=c(0,max(KNN_results_forecasts$k_best[[i]])+3),lwd=lwd_width)
    
        legend("bottom",c("lag 1","lag 2" ,"lag 3","lag 4","lag 5","lag 6"),
         col=c("blue","red","green","yellow","black","brown"),bty="n",cex=1,box.lwd = 2,
           fill=c("blue","red","green","yellow","black","brown"),horiz=F,ncol=2)

    lines(1:k_values,KNN_results_forecasts$k_best[[i]][2,],col="red",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_forecasts$k_best[[i]][3,],col="green",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_forecasts$k_best[[i]][4,],col="yellow",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_forecasts$k_best[[i]][5,],col="black",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_forecasts$k_best[[i]][6,],col="brown",type="l",lwd=lwd_width)
    
    text(30,max(KNN_results_forecasts$k_best[[i]])+3,paste("Forecast origin",":",forecast_origins[i]),col="black",
        font=2)
    }



backcast_origins<- rev(rownames(KNN_results_backcasts$KNN_errors))
par(mfrow=c(3,3),mar=c(4,4,1,1))
k_values<- 52
lead_times<- 1:9
lwd_width<-3
for(i in lead_times){
    plot(1:k_values,KNN_results_backcasts$k_best[[i]][1,],type="l",xlab="K",xlim=c(1,k_values),
         ylab="MAPE (%)",col="blue",ylim=c(0,max(KNN_results_backcasts$k_best[[i]])),lwd=lwd_width)
    
        legend("bottom",c("lag 1","lag 2" ,"lag 3","lag 4","lag 5","lag 6"),
         col=c("blue","red","green","yellow","black","brown"),bty="n",cex=1,box.lwd = 2,
           fill=c("blue","red","green","yellow","black","brown"),horiz=F,ncol=2)

    lines(1:k_values,KNN_results_backcasts$k_best[[i]][2,],col="red",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_backcasts$k_best[[i]][3,],col="green",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_backcasts$k_best[[i]][4,],col="yellow",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_backcasts$k_best[[i]][5,],col="black",type="l",lwd=lwd_width)
    lines(1:k_values,KNN_results_backcasts$k_best[[i]][6,],col="brown",type="l",lwd=lwd_width)
     text(30,max(KNN_results_backcasts$k_best[[i]]),paste("Backcast origin",":",backcast_origins[i]),col="black",
        font=2)
    }
...
LSTM_results_forecasts<-ro_eval_LSTM_forecast(series=demand,forecast_length=18,
                                    lag=1,units = 1,Epochs=50,batch_size = 1,difference=TRUE)
...
ARIMA_results_forecasts_ARIMA_errors[,1])

#Importing the forecast errors for each time series model at different origins
models<- c("ARIMA","KNN","NNAR","GRNN","MLP","ELM","LSTM")

#Forecast errors
ARIMA_results_forecasts_ARIMA_errors <-read.csv("ARIMA_results_forecasts_ARIMA_errors.csv")
KNN_results_forecasts_KNN_errors <-read.csv("KNN_results_forecasts_KNN_errors.csv")
NNAR_results_forecasts_NNAR_errors <-read.csv("NNAR_results_forecasts_NNAR_errors.csv")
GRNN_results_forecasts_GRNN_errors <-read.csv("GRNN_results_forecasts_GRNN_errors.csv")
MLP_results_forecasts_MLP_errors  <-read.csv("MLP_results_forecasts_MLP_errors.csv")
ELM_results_forecasts_ELM_errors <-read.csv("ELM_results_forecasts_ELM_errors.csv")
LSTM_results_forecasts_LSTM_errors  <-read.csv("LSTM_results_forecasts_LSTM_errors.csv")


#Backcast errors
ARIMA_results_backcasts_ARIMA_errors <-read.csv("ARIMA_results_backcasts_ARIMA_errors.csv")
KNN_results_backcasts_KNN_errors <-read.csv("KNN_results_backcasts_KNN_errors.csv")
NNAR_results_backcasts_NNAR_errors <-read.csv("NNAR_results_backcasts_NNAR_errors.csv")
GRNN_results_backcasts_GRNN_errors <-read.csv("GRNN_results_backcasts_GRNN_errors.csv")
MLP_results_backcasts_MLP_errors  <-read.csv("MLP_results_backcasts_MLP_errors.csv")
ELM_results_backcasts_ELM_errors <-read.csv("ELM_results_backcasts_ELM_errors.csv")
LSTM_results_backcasts_LSTM_errors  <-read.csv("LSTM_results_backcasts_LSTM_errors.csv")

# Comparing the forecast errors
par(mar=c(4,4,1,1),mfrow=c(2,1))
plot(ARIMA_results_forecasts_ARIMA_errors[,4],type="b",col="red",lwd=3,
     ylim=c(0,60),xaxt="n",xlab="Forecast origins",ylab="MAPE (%)",main="")
lines(KNN_results_forecasts_KNN_errors[,4],type="b",col="blue",lwd=3)
lines(NNAR_results_forecasts_NNAR_errors[,4],type="b",col="green",lwd=3)
lines(GRNN_results_forecasts_GRNN_errors[,4],type="b",col="yellow",lwd=3)
lines(MLP_results_forecasts_MLP_errors[,4],type="b",col="magenta",lwd=3)
lines(ELM_results_forecasts_ELM_errors[,4],type="b",col="grey",lwd=3)
lines(LSTM_results_forecasts_LSTM_errors[,4],type="b",col="black",lwd=3)
...
plot(rev(ARIMA_results_backcasts_ARIMA_errors[,4]),type="b",col="red",lwd=3,
     ylim=c(0,70),xaxt="n",xlab="Backcast origins",ylab="MAPE (%)",main="")
lines(rev(KNN_results_backcasts_KNN_errors[,4]),type="b",col="blue",lwd=3)
lines(rev(NNAR_results_backcasts_NNAR_errors[,4]),type="b",col="green",lwd=3)
lines(rev(GRNN_results_backcasts_GRNN_errors[,4]),type="b",col="yellow",lwd=3)
lines(rev(MLP_results_backcasts_MLP_errors[,4]),type="b",col="magenta",lwd=3)
lines(rev(ELM_results_backcasts_ELM_errors[,4]),type="b",col="grey",lwd=3)
lines(rev(LSTM_results_backcasts_LSTM_errors[,4]),type="b",col="black",lwd=3)

 axis(1, at=1:17, labels=rev(ARIMA_results_backcasts_ARIMA_errors[,1]))
...
MAPEs_forecasts<-data.frame(ARIMA=ARIMA_results_forecasts_ARIMA_errors[,4],
    KNN=KNN_results_forecasts_KNN_errors[,4],NNAR=NNAR_results_forecasts_NNAR_errors[,4],
    GRNN=GRNN_results_forecasts_GRNN_errors[,4],MLP=MLP_results_forecasts_MLP_errors[,4],
                    ELM=ELM_results_forecasts_ELM_errors[,4],
                LSTM=LSTM_results_forecasts_LSTM_errors[,4])

MAPEs_forecasts$Lead_times<-ARIMA_results_forecasts_ARIMA_errors[,1]
MAPEs_forecasts
...
MAPEs_backcasts<-data.frame(ARIMA=rev(ARIMA_results_backcasts_ARIMA_errors[,4]),
    KNN=rev(KNN_results_backcasts_KNN_errors[,4]),NNAR=rev(NNAR_results_backcasts_NNAR_errors[,4]),
    GRNN=rev(GRNN_results_backcasts_GRNN_errors[,4]),MLP=rev(MLP_results_backcasts_MLP_errors[,4]),
    ELM=rev(ELM_results_backcasts_ELM_errors[,4]),LSTM=rev(LSTM_results_backcasts_LSTM_errors[,4]))

MAPEs_backcasts$Lead_times<-rev(ARIMA_results_backcasts_ARIMA_errors[,1])
MAPEs_backcasts
