...
ro_eval_LSTM_forecast<- function(series,forecast_length,lag=1,units = 1,Epochs=60,
  ...
  if(difference==FALSE) supervised<- lag_transform(series, lag)
  if(difference==TRUE) supervised<- lag_transform(diffed, lag)
  ...
  for(i in lead_time_index){# different forecast origins
    ...
    if(difference==FALSE) N = nf
    if(difference==TRUE)  N= nf-lag
    ...
    for(j in 1:L_test){
      ...
      # invert differencing
      if(difference==FALSE)  yhat  = yhat 
      if(difference==TRUE)  yhat  = yhat + series[(n+j)]
      # store
      predictions[j] <- yhat
    }
    ...
    for(k in 1:L_train){
      ...
      # invert differencing
      if(difference==FALSE)  yhat  = yhat 
      if(difference==TRUE)  yhat  = yhat + series[k]
      # store
      fitted[k] <- yhat
    }
...
ro_eval_LSTM_backcast<- function(series,backcast_length,lag=1,units = 1,Epochs=60,
  ...
  for(i in lead_time_index){# different forecast origins
    ...
    for(j in 1:L_test){
      ...
      # invert differencing
      if(difference==FALSE)  yhat  = yhat 
      if(difference==TRUE)   yhat  = yhat + series[(n-j)+1]
    ...
    for(k in 1:L_train){
      ...
      # invert differencing
      if(difference==FALSE)  yhat  = yhat
      if(difference==TRUE)   yhat  = yhat + series[n+k]
