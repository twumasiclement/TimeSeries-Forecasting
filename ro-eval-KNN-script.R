...
ro_eval_KNN_forecast<- function(series,forecast_length){
  ...
  KNN_Model<-NULL; KNN_Model_forecasts<-NULL;k_best<-NULL
  for(i in lead_time_index){# different forecast origins
    ...
    Out_knn<-KNN_k_lag_forecast_best(train_series=train_series,test_series=test_series,
                            horizon= horizon,lag_values=6,k_values=52)
    ...
    #Saving best K for KNN
    k_best[[i_index]]<-  Out_knn$MAPE#Out_knn$k_selected
  ...
  return(list(KNN_errors=lead_times_errors,KNN_Model=KNN_Model,
          KNN_forecasts= KNN_Model_forecasts,k_best=k_best))
}
...
ro_eval_KNN_backcast<- function(series,backcast_length){
  ...
  return(list(KNN_errors=lead_times_errors,KNN_Model=Reverse_forecast,
          KNN_backcasts= KNN_Model_forecasts,k_best=k_best))
}
